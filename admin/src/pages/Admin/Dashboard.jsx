      ...
      <div className='flex flex-wrap gap-3'>
        ...
        <div className='flex items-center gap-2 bg-white p-4 min-w-52 rounded border-2 border-gray-100 cursor-pointer hover:scale-105 transition-all'>
          ...
          <div>
            ...
            <p className='text-gray-400'>Unique Patients</p>
          </div>
        </div>
        <div className='flex items-center gap-2 bg-white p-4 min-w-52 rounded border-2 border-gray-100 cursor-pointer hover:scale-105 transition-all'>
          <img className='w-14' src={assets.calendar_icon} alt="" />
          <div>
            <p className='text-xl font-semibold text-gray-600'>{dashData.todayAppointments || 0}</p>
            ...
            <div className='flex items-center px-6 py-3 gap-3 hover:bg-gray-100' key={index}>
              <div className='rounded-full w-10 h-10 bg-blue-100 flex items-center justify-center'>
                <span className='text-blue-600 font-semibold'>
                  {item.patientName ? item.patientName.charAt(0).toUpperCase() : 'A'}
                </span>
              </div>
              <div className='flex-1 text-sm'>
                <p className='text-gray-800 font-medium'>{item.patientName || 'Anonymous'}</p>
                <p className='text-gray-600'>Booking on {slotDateFormat(item.slotDate)} at {item.slotTime}</p>
                <p className='text-gray-500 text-xs'>Mobile: {item.patientMobile || 'N/A'}</p>
              </div>
              {item.cancelled ? 
